<?php
	...
	class Modele {
		...
		public function updatePrestataire($tab) {
			$requete = "UPDATE prestataire SET 
							image_prestataire = :image_prestataire, 
			...
			$donnees = array(
				":image_prestataire" => $tab['image_prestataire'],
		...
		public function selectLikePrestataires($filtre1, $filtre2) {
			...
			FROM prestataire p
			INNER JOIN services s ON p.idservice = s.idservice
			WHERE p.zone_couverture LIKE :filtre1 OR p.idservice LIKE :filtre2;";
			$donnees = array(
				":filtre1" => "%" . $filtre1 . "%",
				":filtre2" => "%" . $filtre2 . "%"
			); 
...
public function selectAllReservations() {
	...
	public function insertPrestataire($tab) {
		$requete = "INSERT INTO prestataire (nomprestataire, image_prestataire, adresse, numero_telephone, email, mdp, competences, experience, tarifs, disponibilite, zone_couverture, evaluations, certifications, idservice) 
		VALUES (:nomprestataire, :image_prestataire, :adresse, :numero_telephone, :email, :mdp, :competences, :experience, :tarifs, :disponibilite, :zone_couverture, :evaluations, :certifications, :idservice)";
		$insertion = $this->unPDO->prepare($requete);
		$donnees = array(
			":nomprestataire" => $tab['nomprestataire'],
			":image_prestataire" => $tab['image_prestataire'], // Correction ici
			":adresse" => $tab['adresse'],
			":numero_telephone" => $tab['numero_telephone'],
			":email" => $tab['email'],
			":mdp" => $tab['mdp'],
		...
		$insertion->execute($donnees); // Correction ici
	}
