...
if (isset($_POST['Connexion'])) {
    ...
    if ($statut == "client") {
        ...
        $unUser = $unControleur->verifConnexionPrestataire($email, $mdp);
        if ($unUser != null) {
            $_SESSION['iduser'] = $unUser['idprestataire'];
            $_SESSION['email'] = $unUser['email'];
            $_SESSION['nom'] = $unUser['nomprestataire'];
            $_SESSION['statut'] = $statut;
            $_SESSION['role'] = "user";
            header("Location: home.php"); // Redirection vers home.php
        } else {
            echo "<br> Veuillez vérifier vos identifiants.";
        }
    } else {
